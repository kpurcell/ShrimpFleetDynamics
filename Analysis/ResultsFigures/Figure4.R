...
#DO
plot(nspm.2mo.avg.dur.la, select=1, shade=T, all.terms=T, scale=0, rug=T,
...
plot(nspm.2mo.avg.dur.tx, select=1, shade=T, all.terms=T, scale=0,rug=T,
     xlab=expression(paste("Dissolved Oxygen"~ (mgL^{-1}))),
     ylab="s(Average Tow Duration)",
     ylim=c(-0.35,0.23),
     main="Texas")
...
#DO
plot(nspm.2mo.tow.count.la, select=1, shade=T, all.terms=T, scale=0,rug=T,
     xlab=expression(paste("Dissolved Oxygen"~ (mgL^{-1}))),
     ylab="s(Tow Density)",
     ylim=c(-1.2,0.22))
#abline(h=0)
text(0.55,0.22, "(c)", cex=1,font=2)
plot(nspm.2mo.tow.count.tx, select=1, shade=T, all.terms=T, scale=0,rug=T,
     xlab=expression(paste("Dissolved Oxygen"~ (mgL^{-1}))),
     ylab="s(Tow Density)")
#abline(h=0)
